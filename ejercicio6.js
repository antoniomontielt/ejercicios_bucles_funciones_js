...
const starWarsMovies = [
  ...
  let starWarsByDecade = {};

  for(const movie of starWarsMovies) {
    let decade = (movie.releaseYear - (movie.releaseYear % 10));

    if (!starWarsByDecade[decade]) {
        starWarsByDecade[decade] = [];
    }
    starWarsByDecade[decade].push(movie.title);
  }

  console.log(starWarsByDecade);